...
export function getIcon(value = "", variant = "category", className = "w-[17px] h-[17px]") {
  if (variant !== "category") {
    const Icon = ICONS[variant];
    return Icon ? <Icon className={className} /> : <TagIcon className={className} />;
  }
  ...
  if (name.includes("opinion"))
    return <OpinionIcon className={className} />;

  if (name.includes("press"))
    return <PressReleaseIcon className={className} />;
